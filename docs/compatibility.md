...
| 11.11.1 at `/company/chat` | Native REST/WebSocket and conversation/attachment checks with URL-subpath preservation; socket-gap recovery. |
| 10.11.24 at origin root | Native REST/WebSocket and conversation/attachment checks on the 10.11 release line; socket-gap recovery. |
...
| Reactions/search | Add/remove reactions; team post search; system (Unicode) emoji rendering, `:` completion and a native reaction picker from a static table of Mattermost v11.11.1 `SystemEmojis` names; server custom emoji in messages, reactions, colon completion and the paged Custom picker when enabled; reaction chips name up to ten reactors in their tooltip and accessibility help ("You, bob and 3 others reacted with :+1:") | Animated custom emoji show their first frame; browsing retains 600 custom entries (search reaches others); reaction names over 64 characters are not offered; the picker's frequently used row and the hover bar's quick reactions are fixed lists; reactors whose profiles are not loaded yet are counted, not named; advanced search UI remains incomplete. |
| Users | Own-profile first/last name, nickname and position editing; bounded center-cropped profile-picture upload/removal; Out of Office presence and DND end times; batched users/statuses, `POST /users/usernames`, autocomplete, profile images; profile cards (position, local time zone, custom status, email when the server exposes it); `PUT /users/{id}/status` and `PUT`/`DELETE /users/{id}/status/custom` for the signed-in user; names follow `TeammateNameDisplay` / `LockTeammateNameDisplay` and the `name_format` preference | Bounded results; not a full administrative directory. Custom status emoji are system emoji names only. |
| Notifications | Dock badge with unread mention count, or a dot for unread messages without mentions. Alerts follow the account's `notify_props` (`desktop`, `desktop_sound`, `mention_keys`, `first_name`, `channel`) from `/users/me` and `user_updated`, and each channel member's `desktop`, `mark_unread` (mute) and `ignore_channel_mentions`: the server's `mentions` list plus a client-side whole-word, case-insensitive keyword match. Notification Center alerts, on by default (sender and conversation, plus up to 100 characters of text while the default-on preview setting is enabled), an in-app system sound and one Dock bounce for mentions/DMs while inactive. Suppressed during Do Not Disturb and for the conversation on screen; clicking opens it. Account level, sound, keywords, first-name and channel-wide triggers are editable in Settings (`PUT /users/{id}/patch` with the complete `notify_props` map) | Local switches (Notification Center, preview, sound choice, Dock bounce) are saved on this Mac (decision 0032); macOS permission is asked once, after sign-in; no new alerts after quitting. Normal quit and sign-out request removal of pending and delivered alerts, but macOS may retain them after a crash or forced termination. Retained alerts cannot navigate a later app instance. With collapsed threads, channel replies notify for mentions or the server-selected desktop followers (`posted.followers`, already filtered by `desktop_threads` and channel preferences). Older servers omitting that field retain mention-only behavior. In-app sounds follow Mattermost Do Not Disturb, not macOS Focus. Notify-prop maps over 48 keys or 4 KB per value are not editable. Push and email settings are not shown. |
...
| File search | Team file search and Channel Files, paginated bounded results, image previews, explicit Save and Jump to Message | Server grammar; no local index. Actions require channel metadata and membership. |
| Realtime | `/websocket`, authentication, event reconciliation and reconnect; live socket-gap tests recover missed posts, edits and deletions on all three deployments while preserving a draft | Unknown send outcomes remain visible; no exactly-once delivery guarantee. Physical Mac sleep/wake is not covered by the socket-gap test. |
...
The historical SwiftUI sidebar reentrancy warning did not recur in the latest
combined run, but that does not prove all paths free of it. The AppIntents
metadata-extraction warning remains. See the dated progress log.
