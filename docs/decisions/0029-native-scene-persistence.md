...
only the known native geometry key, under a 1 MiB input cap, and prints change
...
without successful window resizing used an incorrectly split key name and cannot
establish geometry stability. The corrected key is the single literal
`NSSplitView Subview Frames main, SidebarNavigationSplitView`. The guard is not
