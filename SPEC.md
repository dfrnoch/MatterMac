...
Release diagnostics stay in a bounded in-memory ring containing redacted categories, durations, counters, and non-sensitive codes. No automatic file logs, unified-log message contents, telemetry, or crash uploads.

In-app updates (user request 2026-09-25, decision 0034) are the only contact with a host other than the user's Mattermost servers. Release builds check MatterMac's public GitHub releases (`api.github.com`, and GitHub's download hosts for release assets) at launch and every six hours when "Check for updates automatically" is on, or on request. No account, server or message data is sent. Updates are verified by SHA-256, Developer ID signature, notarization, bundle identifier and build number, first in the app and again in the embedded installer XPC service, before the app bundle is replaced. The installer is the only unsandboxed code; it accepts only MatterMac signed by the same team. Developer-enabled Instruments captures and explicitly requested test reports are separate from normal user operation.
